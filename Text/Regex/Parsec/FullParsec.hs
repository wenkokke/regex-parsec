...
import Text.Regex.Parsec.RegexParsecState(lookupAccepted, initState, eqSubs,
  lookupSubs, lookupSub, stopSub, startSub, plusState, incState, finalState)
 ...
 where
  ...
  reflectParsec pIn cont =
    case pIn of
      ...
      PGroup i p -> if captureG then startSub i >> reflectParsec p (stopSub i >> cont)
