...
import Text.ParserCombinators.Parsec (GenParser, anyChar, eof, getInput, option, runParser, try, (<|>))
import Text.Regex.Base.RegexLike (RegexContext (..), RegexMaker (..), RegexOptions (..))
...
import Text.Regex.Parsec.FullParsec (hasFrontCarat, patternToParsec)
import Text.Regex.Parsec.ReadRegex (parseRegex)
import Text.Regex.Parsec.RegexParsecState (getUserState, incState, lookupAccepted, newState, updateUserState)
...
wrapCount r@(Regex {userInt = regex, frontAnchor = anchored}) source =
  let parser =
        (try regex >> updateUserState succ >> parser)
          <|> (anyChar >> parser)
          <|> (eof >> getUserState)
   in if anchored -- punt to wrapMatch
