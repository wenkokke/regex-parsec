...
import qualified Data.IntMap as I (empty, fromAscList, insert, lookup, toAscList, updateLookupWithKey)
import Text.ParserCombinators.Parsec (getInput, getState, setState, updateState)
import Text.Regex.Parsec.Common (Closed (..), FullState (..), MatchedStrings, Opened (..), RegexParser)
...
initState = do
  -- keep only userState and accepted, reset capture
...
lookupSub i = do
  msol <- liftM ((I.lookup i) . closedSub) getState
  case msol of
...
eqSubs a b =
  let a' = map (\(k, (_, ol)) -> (k, ol)) . I.toAscList $ a
...
lookupSubsP =
  let traverseOpen acc (EndOpened closed) = traverseClosed acc closed
   ...
   in do
        (FullState {posixSub = sub}) <- getState
