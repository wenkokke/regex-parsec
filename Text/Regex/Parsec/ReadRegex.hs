...
import Data.Set (Set)
import qualified Data.Set as Set (empty, fromList, insert, toList)
...
p_post_atom_ng :: Pattern -> GenParser Char PatternIndex Pattern
p_post_atom_ng atom =
  ((char '?' <?> "lazy ? modifier") >> return (PLazy atom))
    <|> ((char '+' <?> "possessive + modifier") >> return (PPossessive atom))
    <|> return atom

p_char = p_dot <|> p_left_brace <|> p_escaped <|> p_other_char
  where
    p_dot = char '.' >> return PDot
...
p_set invert = do
  initial <- (option "" ((char ']' >> return "]") <|> (char '-' >> return "-")))
